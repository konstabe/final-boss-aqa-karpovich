...
export class CreateOrderModal extends BaseModal {
	...
	async selectCustomerAndProduct(
		initialCustomerName: string,
		initialProductName: string,
	): Promise<{ customerName: string; productName: string }> {
		const customerNames = await this.customerField.selectOption(initialCustomerName);
		const productNames = await this.productField.selectOption(initialProductName);

		const customerName = customerNames[0] || initialCustomerName;
		const productName = productNames[0] || initialProductName;
